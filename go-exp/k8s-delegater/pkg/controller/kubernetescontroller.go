...
func CheckFlinkJobRequest(request request.FlinkJobRequest) {
	...
	if request.ArtifactId == "" {
		common.GetLog().Error("artifact id is null!")
		panic(response.Fail(http.StatusInternalServerError, "artifact id is null", nil))
	}

	if request.BuildId == "" {
		common.GetLog().Error("build id is null!")
		panic(response.Fail(http.StatusInternalServerError, "build id is null", nil))
	}

	if request.DWLayer == "" {
		common.GetLog().Error("dw layer id is null!")
		panic(response.Fail(http.StatusInternalServerError, "dw layer is null", nil))
	}

	if request.Application == "" {
		common.GetLog().Error("application id is null!")
		panic(response.Fail(http.StatusInternalServerError, "application is null", nil))
	}

	if request.ClusterId == "" {
		common.GetLog().Error("cluster id is null!")
		panic(response.Fail(http.StatusInternalServerError, "cluster id is null", nil))
	}

	if len(request.ClusterId) > 45 {
		common.GetLog().Error("cluster-id must be no more than 45 characters!")
		panic(response.Fail(http.StatusInternalServerError, "cluster-id must be no more than 45 characters", nil))
	}
}
